...
**`inspect` results are cached on disk and survive across sessions — but only for an exact repeat.** A cache hit costs nothing. **Check `_cache.enabled` first: when it is `false`, the cache is turned off (`FIGMA_READER_CACHE=off`) and every call — repeats included — costs a request.** A hit requires the file key, the *set* of node ids, `--depth`, and `--geometry` to all match a previous call. Change any one of them and you pay full price:
...
- **When the cache cannot be written, nothing new is ever stored.** If `_cache.note` says `Writing the cache under <dir> failed (<code>)`, the cache directory is not writable — typically because a sandbox denies writes to the home directory. Repeats cost a full request, so plan as if there were no cache. The one exception makes things worse, not better: when the note also says an older cached response could not be removed, a repeat without `--refresh` may silently return that **stale** response instead of calling the API. Tell the user what the note says, including the directory and the fix it suggests. **Do not edit sandbox or settings files yourself**: widening what the sandbox may write is the user's decision.
...
This is about *your* `jq` scratch file, not about re-fetching. Re-running `inspect` on the same node is free (cache hit) — unless `_cache.enabled` is `false` or the cache could not be written, in which case a re-run costs a request (or, if the note says an older response could not be removed, may return that stale one) — so re-run it rather than reaching for a file some earlier session left behind. Staleness of the design data itself is now reported by the CLI in `_cache` — see hard rule 8.
...
**`_cache` is present on every response**, whether the data came from the API or from disk. `hit` says which; `ageSeconds` is how old the data is; `cached` says whether this call's response is stored on disk; `enabled` says whether the cache is turned on at all; `note` restates it all in prose. **When `cached` is `false`, repeating the request usually costs another call** — the response was not stored, because the cache is turned off (`enabled: false`), because the write failed (the note then names the error code, the directory, and the fix), or because some requested id did not resolve. The exception: when the note says an older cached response could not be removed, that stale response is still on disk and a repeat without `--refresh` may be served from it. Report the age to the user on a hit — see hard rule 8.
...
Cache files live in `$FIGMA_READER_CACHE_DIR/` when that variable is set, otherwise in `$XDG_CACHE_HOME/figma-reader/` when that variable holds an absolute path, otherwise in `~/.cache/figma-reader/`. Deleting the default directory (`~/.cache/figma-reader/` or `$XDG_CACHE_HOME/figma-reader/`) is safe; the next call just fetches again. **Never delete a `$FIGMA_READER_CACHE_DIR` directory yourself**: files are placed directly under it, and it may be shared with other data — leave that to the user. Do not delete it to "get fresh data" — that throws away every file's cache to refresh one, and `--refresh` does that for a single request.
