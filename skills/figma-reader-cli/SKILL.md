...
- **When the cache cannot be written, nothing is ever a hit.** If `_cache.note` says `Writing the cache under <dir> failed (<code>)`, the cache directory is not writable — typically because a sandbox denies writes to the home directory. Every repeat in this session will cost a full request, so plan as if there were no cache. Tell the user what the note says, including the directory and the fix it suggests. **Do not edit sandbox or settings files yourself**: widening what the sandbox may write is the user's decision.
...
Cache files live in `$FIGMA_READER_CACHE_DIR/` when that variable is set, otherwise in `$XDG_CACHE_HOME/figma-reader/` when that variable holds an absolute path, otherwise in `~/.cache/figma-reader/`. Deleting that cache directory is safe; the next call just fetches again. Do not delete it to "get fresh data" — that throws away every file's cache to refresh one, and `--refresh` does that for a single request.
